...
export default class LatestCommand extends Command {
  ...
  private makeFullEmbed(): MessageEmbed {
    const codeBlockWindows = discordCodeBlock(
      JSON.stringify(this.versionData.windows, null, 2)
    );

    const codeBlockLinux = discordCodeBlock(
      JSON.stringify(this.versionData.linux, null, 2)
    );
